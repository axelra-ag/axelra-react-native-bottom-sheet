...
export const useGestureEventsHandlersDefault = () => {
    //#region variables
    const { animatedPosition, animatedSnapPoints, animatedKeyboardState, animatedKeyboardHeight, animatedContainerHeight, animatedScrollableType, animatedHighestSnapPoint, animatedClosedPosition, animatedScrollableContentOffsetY, enableOverDrag, enablePanDownToClose, overDragResistanceFactor, isInTemporaryPosition, isScrollableRefreshable, animateToPosition, stopAnimation, } = useBottomSheetInternal();
    const context = useSharedValue({
        ...INITIAL_CONTEXT,
    });
    //#endregion
    //#region gesture methods
    const handleOnStart = useWorkletCallback(function handleOnStart(__, { translationY }) {
    ...
    const handleOnChange = useWorkletCallback(function handleOnChange(source, { translationY }) {
         ...
         */
        if (source === GESTURE_SOURCE.CONTENT &&
         ...
         */
        const negativeScrollableContentOffset = (context.value.initialPosition === highestSnapPoint &&
        ...
        if (enableOverDrag) {
            if ((source === GESTURE_SOURCE.HANDLE ||
                ...
                const resistedPosition = highestSnapPoint -
            ...
            if (source === GESTURE_SOURCE.HANDLE &&
                ...
                const resistedPosition = lowestSnapPoint +
    ...
    const handleOnEnd = useWorkletCallback(function handleOnEnd(source, { translationY, absoluteY, velocityY }) {
         ...
         */
        const isScrollable = animatedScrollableType.value !== SCROLLABLE_TYPE.UNDETERMINED &&
            animatedScrollableType.value !== SCROLLABLE_TYPE.VIEW;
        /**
         * if keyboard is shown and the sheet is dragged down,
         * then we dismiss the keyboard.
         */
        if (context.value.initialKeyboardState === KEYBOARD_STATE.SHOWN &&
        ...
        const wasGestureHandledByScrollView = source === GESTURE_SOURCE.CONTENT &&
