...
const BottomSheetComponent = forwardRef(function BottomSheet(props, ref) {
    ...
    // styles
    style: _providedStyle, containerStyle: _providedContainerStyle, backgroundStyle: _providedBackgroundStyle, handleStyle: _providedHandleStyle, handleIndicatorStyle: _providedHandleIndicatorStyle, 
    // hooks
    gestureEventsHandlersHook, 
    // keyboard
    keyboardBehavior = DEFAULT_KEYBOARD_BEHAVIOR, keyboardBlurBehavior = DEFAULT_KEYBOARD_BLUR_BEHAVIOR, android_keyboardInputMode = DEFAULT_KEYBOARD_INPUT_MODE, 
    // layout
    containerHeight: _providedContainerHeight, containerOffset: _providedContainerOffset, topInset = 0, bottomInset = 0, maxDynamicContentSize, 
    // animated callback shared values
    animatedPosition: _providedAnimatedPosition, animatedIndex: _providedAnimatedIndex, 
    // gestures
    simultaneousHandlers: _providedSimultaneousHandlers, waitFor: _providedWaitFor, activeOffsetX: _providedActiveOffsetX, activeOffsetY: _providedActiveOffsetY, failOffsetX: _providedFailOffsetX, failOffsetY: _providedFailOffsetY, 
    // callbacks
    onChange: _providedOnChange, onClose: _providedOnClose, onAnimate: _providedOnAnimate, 
    // private
    $modal = false, detached = false, 
    // components
    handleComponent, backdropComponent, backgroundComponent, footerComponent, children, customIndicatorComponent, 
    // accessibility
    accessible: _providedAccessible = DEFAULT_ACCESSIBLE, accessibilityLabel: _providedAccessibilityLabel = DEFAULT_ACCESSIBILITY_LABEL, accessibilityRole: _providedAccessibilityRole = DEFAULT_ACCESSIBILITY_ROLE, } = props;
    //#endregion
    ...
    const isLayoutCalculated = useDerivedValue(() => {
        ...
        if (_providedContainerHeight !== null ||
        ...
        return (isContainerHeightCalculated &&
    ...
    const { animatedScrollableType, animatedScrollableContentOffsetY, animatedScrollableOverrideState, isScrollableRefreshable, setScrollableRef, removeScrollableRef, } = useScrollable();
    // keyboard
    const { state: animatedKeyboardState, height: animatedKeyboardHeight, animationDuration: keyboardAnimationDuration, animationEasing: keyboardAnimationEasing, shouldHandleKeyboardEvents, } = useKeyboard();
    ...
    const animatedSheetState = useDerivedValue(() => {
        ...
        const extendedPositionWithKeyboard = Math.max(0, animatedContainerHeight.value -
    ...
    const animatedContentHeightMax = useDerivedValue(() => {
        ...
        if (keyboardBehavior === KEYBOARD_BEHAVIOR.extend &&
            ...
            if (animatedKeyboardState.value === KEYBOARD_STATE.SHOWN) {
                if (keyboardHeightInContainer + animatedSheetHeight.value >
                    animatedContainerHeight.value) {
                    contentHeight =
                        animatedContainerHeight.value -
                            keyboardHeightInContainer -
            ...
            else if (contentWithKeyboardHeight + handleHeight >
                animatedContainerHeight.value) {
                contentHeight = animatedContainerHeight.value - handleHeight;
            }
            else {
    ...
    const animatedIndex = useDerivedValue(() => {
         ...
         */
        if (animatedAnimationSource.value === ANIMATION_SOURCE.SNAP_POINT_CHANGE &&
            animatedAnimationState.value === ANIMATION_STATE.RUNNING) {
    ...
    const getNextPosition = useWorkletCallback(function getNextPosition() {
         ...
         */
        if (keyboardBlurBehavior === KEYBOARD_BLUR_BEHAVIOR.restore &&
         ...
         */
        if (keyboardBehavior === KEYBOARD_BEHAVIOR.fillParent &&
            keyboardState === KEYBOARD_STATE.SHOWN) {
    ...
    const handleOnChange = useCallback(function handleOnChange(index, position) {
        ...
        if (_providedOnChange) {
            _providedOnChange(index, position, index === animatedDynamicSnapPointIndex.value
    ...
    const setToPosition = useWorkletCallback(function setToPosition(targetPosition) {
        if (targetPosition === animatedPosition.value ||
    ...
    const handleForceClose = useCallback(function handleForceClose(animationConfigs) {
         ...
         * - sheet is forced closing.
         */
        if (nextPosition === animatedNextPosition.value ||
            isForcedClosing.value) {
            return;
        }
        /**
    ...
    useAnimatedReaction(() => ({
        ...
        if (JSON.stringify(snapPoints) === JSON.stringify(_previousSnapPoints) ||
    ...
    useAnimatedReaction(() => ({
         ...
         */
        const hasActiveGesture = animatedContentGestureState.value === State.ACTIVE ||
    ...
    useAnimatedReaction(() => ({
    ...
    }), ({ _animatedIndex, _animatedPosition, _animationState, _contentGestureState, _handleGestureState, }) => {
        /**
         ...
         */
        if (animatedNextPosition.value !== INITIAL_VALUE &&
            animatedNextPositionIndex.value !== INITIAL_VALUE &&
         ...
         */
        const hasNoActiveGesture = (_contentGestureState === State.END ||
    ...
    return (React.createElement(BottomSheetProvider, { value: externalContextVariables },
        React.createElement(BottomSheetInternalProvider, { value: internalContextVariables },
            React.createElement(BottomSheetGestureHandlersProvider, { gestureEventsHandlersHook: gestureEventsHandlersHook },
                React.createElement(BottomSheetBackdropContainer, { key: "BottomSheetBackdropContainer", animatedIndex: animatedIndex, animatedPosition: animatedPosition, backdropComponent: backdropComponent }),
                React.createElement(BottomSheetContainer, { key: "BottomSheetContainer", shouldCalculateHeight: !$modal, containerHeight: _animatedContainerHeight, containerOffset: animatedContainerOffset, topInset: topInset, bottomInset: bottomInset, detached: detached, style: _providedContainerStyle },
                    React.createElement(Animated.View, { style: containerStyle },
                        React.createElement(BottomSheetBackgroundContainer, { key: "BottomSheetBackgroundContainer", animatedIndex: animatedIndex, animatedPosition: animatedPosition, backgroundComponent: backgroundComponent, backgroundStyle: _providedBackgroundStyle }),
                        React.createElement(Animated.View, { pointerEvents: "box-none", style: contentMaskContainerStyle, accessible: _providedAccessible ?? undefined, accessibilityRole: _providedAccessibilityRole ?? undefined, accessibilityLabel: _providedAccessibilityLabel ?? undefined },
                            React.createElement(BottomSheetDraggableView, { key: "BottomSheetRootDraggableView", style: contentContainerStyle },
                                children,
                                footerComponent && (React.createElement(BottomSheetFooterContainer, { footerComponent: footerComponent })))),
                        React.createElement(BottomSheetHandleContainer, { key: "BottomSheetHandleContainer", animatedIndex: animatedIndex, animatedPosition: animatedPosition, handleHeight: animatedHandleHeight, enableHandlePanningGesture: enableHandlePanningGesture, enableOverDrag: enableOverDrag, enablePanDownToClose: enablePanDownToClose, overDragResistanceFactor: overDragResistanceFactor, keyboardBehavior: keyboardBehavior, handleComponent: handleComponent, handleStyle: _providedHandleStyle, handleIndicatorStyle: _providedHandleIndicatorStyle, customIndicatorComponent: customIndicatorComponent })))))));
});
